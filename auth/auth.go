...
import (
	...
	"database/sql"
	"errors"
	"github.com/if-itb/siasn-idis-backend/libs/ec"
	"github.com/if-itb/siasn-idis-backend/libs/httputil"
	"github.com/if-itb/siasn-idis-backend/libs/logutil"
	"github.com/if-itb/siasn-idis-backend/libs/metricutil"
	"net/http"
	"net/url"
	"strings"
	"time"
...
type Asn struct {
	...
	WorkSatkerId                string `json:"satker_kerja_id"`
	WorkSatker                  string `json:"satker_kerja"`
...
select 
       ...
       coalesce(satuan_kerja_kerja_nama, ''),
...
func (a *Auth) OidcHandler(writer http.ResponseWriter, request *http.Request) {
	...
	a.CookieSetter.SetSessionCookie(writer, rawIdToken, token.Expiry.Sub(time.Now()))
...
func (a *Auth) LogoutHandler(writer http.ResponseWriter, request *http.Request) {
	token := a.CookieSetter.GetSessionCookie(request)
	...
	a.CookieSetter.DeleteSessionCookie(writer)

	logoutUrl, _ := url.Parse(a.EndSessionEndpoint)
	q := url.Values{}
	q.Set("id_token_hint", token)
	q.Set("state", uuid.New().String())
	q.Set("redirect_uri", a.SuccessRedirectUrl)
...
func (a *Auth) VerifyUserinfo(writer http.ResponseWriter, request *http.Request) (user *User, err error) {
	...
	_, user, err = a.VerifyRawIdToken(token)
	if err != nil {
		a.Logger.Warnf("cannot verify ID token: %s", err)
		a.WriteUnauthorized(writer, ErrCodeNoIdToken, ErrMessageNoIdToken, err)
		return
...
// HttpUserExtendedAuth authenticates and retrieves user extended data.
// See also UserExtendedAuthHandler.
func (a *Auth) HttpUserExtendedAuth(writer http.ResponseWriter, request *http.Request) (user *User, err error) {
	rawIdTokenCookie, err := request.Cookie("token")
	if err != nil || rawIdTokenCookie == nil || rawIdTokenCookie.Value == "" {
		a.WriteUnauthorized(writer, ErrCodeNoIdToken, ErrMessageNoIdToken, err)
		return
	...
	if err != nil {
		a.Logger.Warnf("cannot verify ID token: %s", err)
		a.WriteUnauthorized(writer, ErrCodeNoIdToken, ErrMessageNoIdToken, err)
		return nil, ec.NewError(ErrCodeNoIdToken, ErrMessageNoIdToken, err)
	}

	accessToken, err := a.AccessTokenCache.GetAccessToken(rawIdTokenCookie.Value)
	if err != nil {
		a.Logger.Warnf("cannot retrieve user access token: %s", err)
		err = ec.NewError(ErrCodeCannotVerifyIdToken, ErrMessageCannotVerifyIdToken, err)
		_ = httputil.WriteObj(writer, err, http.StatusInternalServerError)
		return nil, err
	}
	if accessToken == nil {
		a.WriteUnauthorized(writer, ErrCodeNoUserInSessionStorage, ErrMessageNoUserInSessionStorage, err)
		return nil, ec.NewError(ErrCodeNoUserInSessionStorage, ErrMessageNoUserInSessionStorage, err)
	}
	user.AccessToken = accessToken
...
// HttpUserDetailAuth authenticates and retrieves user detail.
// See also UserDetailAuthHandler. User extended detail is first retrieved.
func (a *Auth) HttpUserDetailAuth(writer http.ResponseWriter, request *http.Request) (userDetail *Asn, err error) {
	user, err := a.HttpUserExtendedAuth(writer, request)
	if err != nil {
		return nil, err
	}
	...
	if userDetail.WorkAgencyId == "" {
		err = ec.NewErrorBasic(ErrCodeNoWorkAgencyId, ErrMessageNoWorkAgencyId)
		_ = httputil.WriteObj(writer, err, http.StatusForbidden)
		return nil, err
	}

	if user == nil {
		err = ec.NewErrorBasic(ErrCodeInvalidUser, ErrMessageInvalidUser)
		_ = httputil.WriteObj(writer, err, http.StatusForbidden)
		return nil, err
	}
